...
const KNOWN_MOVE_DEFS = {
  ...
  knight: { isRepeatable: false, canJump: true, vectors: [[-1, -2, 1, false, false], [1, -2, 1, false, false]] },
  silver: { isRepeatable: false, canJump: false, vectors: [[-1, -1, 1, false, false], [0, -1, 1, false, false], [1, -1, 1, false, false], [-1, 1, 1, false, false], [1, 1, 1, false, false]] },
  gold: { isRepeatable: false, canJump: false, vectors: [[-1, -1, 1, false, false], [0, -1, 1, false, false], [1, -1, 1, false, false], [-1, 0, 1, false, false], [1, 0, 1, false, false], [0, 1, 1, false, false]] },
  bishop: { isRepeatable: true, canJump: false, vectors: [[-1, -1, 8, false, false], [1, -1, 8, false, false], [-1, 1, 8, false, false], [1, 1, 8, false, false]] },
  rook: { isRepeatable: true, canJump: false, vectors: [[0, -1, 8, false, false], [0, 1, 8, false, false], [-1, 0, 8, false, false], [1, 0, 8, false, false]] },
  king: { isRepeatable: false, canJump: false, vectors: [[-1, -1, 1, false, false], [0, -1, 1, false, false], [1, -1, 1, false, false], [-1, 0, 1, false, false], [1, 0, 1, false, false], [-1, 1, 1, false, false], [0, 1, 1, false, false], [1, 1, 1, false, false]] },
  run: { isRepeatable: true, canJump: false, vectors: [[0, -1, 2, false, false]] },
  p: { isRepeatable: false, canJump: false, vectors: [[0, -1, 1, false, false], [0, 1, 1, false, false], [-1, 0, 1, false, false], [1, 0, 1, false, false]] },
  dance: { isRepeatable: false, canJump: false, vectors: [[-1, -1, 1, false, false], [0, -1, 1, false, false], [1, -1, 1, false, false], [-1, 0, 1, false, false], [1, 0, 1, false, false], [0, 1, 1, false, false]] },
  cry: { isRepeatable: false, canJump: false, vectors: [[-1, -1, 1, false, false], [0, -1, 1, false, false], [1, -1, 1, false, false], [-1, 1, 1, false, false], [1, 1, 1, false, false]] },
};
...
function generateSql(pieces, movePatterns, skills) {
  ...
  const moveRows = movePatterns
    .map((m) => `(${escapeSql(m.moveCode)}, ${escapeSql(m.moveName)}, ${m.isRepeatable}, ${m.canJump}, ${escapeSql(m.constraintsJson ? JSON.stringify(m.constraintsJson) : null)}::jsonb, true, now(), now())`)
    .join(',\n  ');
...
`;
  ...
  const vectorRows = movePatterns
    .flatMap((m) => m.vectors.map((v) => `(${escapeSql(m.moveCode)}, ${v[0]}, ${v[1]}, ${v[2]}, ${v[3]}, ${v[4]})`))
    .join(',\n  ');
...
`;
  ...
  const skillValues = [...skills.values()]
    .map((s) => `(${escapeSql(s.skillCode)}, ${escapeSql(s.skillName)}, ${escapeSql(s.skillDesc)}, ${escapeSql(s.triggerTiming)}, true, now(), now())`)
    .join(',\n  ');
...
`
  ...
  const pieceValues = pieces
    .map((p) => {
      const skillCode = p.skillDesc && p.skillDesc !== 'なし' && p.skillDesc !== '-' ? `skill_${hash12(p.skillDesc)}` : null;
...
function buildSupabaseRestClient() {
  ...
  if (!baseUrl || !serviceRoleKey) {
    throw new Error('Missing SUPABASE_URL (or EXPO_PUBLIC_SUPABASE_URL) or SUPABASE_SERVICE_ROLE_KEY.');
  }

  async function request(pathname, { method = 'GET', body, query = '', schema = 'master', prefer } = {}) {
...
async function main() {
  ...
  console.log(`[info] Pieces: ${pieces.length}, MovePatterns: ${movePatterns.length}, Skills: ${skills.size}`);
