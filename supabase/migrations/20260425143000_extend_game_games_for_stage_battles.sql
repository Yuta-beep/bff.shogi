...
alter table game.games
  ...
  add column if not exists finish_payload jsonb not null default '{}'::jsonb,
  add column if not exists finish_summary jsonb,
  add column if not exists final_snapshot_hash text,
  add column if not exists reward_granted_at timestamptz;

create index if not exists games_stage_session_player_idx
  on game.games(player_id, status, stage_id, started_at desc);
...
begin
  ...
  if v_game.status = 'aborted' or v_game.expires_at is null or v_game.expires_at <= now() then
    update game.games
    set
      status = 'aborted',
      result = 'abort',
