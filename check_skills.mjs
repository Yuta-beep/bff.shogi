...
const { data: skills } = await supabase
  .from('m_skill')
  .select(
    'skill_id, skill_code, skill_desc, skill_type, target_rule, effect_summary_type, trigger_timing, parse_status, proc_chance, duration_turns, params_json',
  )
  .order('skill_id');
...
for (const s of skills) {
  ...
  lines.push(
    `| ${s.skill_id} | ${pieces} | ${s.skill_desc} | ${s.skill_type} | ${s.target_rule} | ${s.effect_summary_type} | ${s.trigger_timing} | ${proc} | ${dur} | ${s.parse_status} |`,
  );
}
