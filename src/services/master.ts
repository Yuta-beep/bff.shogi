...
export async function getStageBattleSetup(stageId: number) {
  const placementRes = await supabaseAdmin
    ...
    .select('side,row_no,col_no,piece_id,m_piece:piece_id(piece_code,kanji,name,move_pattern_id,skill_id)')
    .eq('stage_id', stageId)
    ...
    board: {
      ...
      placements: (placementRes.data ?? []).map((row: any) => ({
        ...
        piece: {
          ...
          movePatternId: row.m_piece?.move_pattern_id ?? null,
          skillId: row.m_piece?.skill_id ?? null,
        },
      })),
    },
