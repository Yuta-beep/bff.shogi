...
type StageListDeps = {
  listPublishedStages: typeof listPublishedStages;
};
...
export function createGetStageList(deps: StageListDeps = { listPublishedStages }) {
  return async function getStageList() {
    try {
      const stages = await deps.listPublishedStages();
...
export const getStageList = createGetStageList();
